...
def main(config):
    ...
    if config.dataset == 'CelebA':
        ...
        dataset = CelebA(config.image_dir, config.selected_attrs, transform, config.mode)

    elif config.dataset == 'mnist2mnistm':
...
if __name__ == '__main__':
    ...
    # Directories.
    parser.add_argument('--image_dir', type=str, default='data/celeba')
    # parser.add_argument('--attr_path', type=str, default='data/list_attr_celeba.txt')
    parser.add_argument('--exp_dir', type=str, default='.')
    parser.add_argument('--exp_name', type=str, default='stargan')
    ...
    config.model_save_dir = os.path.join(config.exp_dir, config.exp_name, 'models')
    config.sample_dir = os.path.join(config.exp_dir, config.exp_name, 'samples')
    config.result_dir = os.path.join(config.exp_dir, config.exp_name, 'results')
